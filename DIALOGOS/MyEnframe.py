...
def enframe(signal, frameLen, frameStep):
  ...
  '''
    ...
    if sLen <= frameLen:
        ...
        numFrames = 1 + int(
            math.ceil((1.0 * sLen - frameLen) / frameStep))  # ceil return a int that not less than input

    padLen = int((numFrames - 1) * frameStep + frameLen)
    zeros = np.zeros((padLen - sLen,))
    padsignal = np.concatenate((signal, zeros))

    indices = np.tile(np.arange(0, frameLen), (numFrames, 1)) + np.tile(np.arange(0, numFrames * frameStep, frameStep),
                                                                        (frameLen, 1)).T  # T is Transpose
